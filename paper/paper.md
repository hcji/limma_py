...
![Figure 1. Statistical consistency between `limma_py` and R limma on protein thermal stability datasets.(A) Comparison of log fold change estimates. (B) Comparison of p-values. Each point represents a protein, with values from `limma_py` plotted against those from R limma.](fig_1.png)
...
![Figure 2. Computational performance comparison between `limma_py` and R limma. (A) Execution time by analysis step. (B) Overall runtime comparison across datasets.](fig_2.png)
